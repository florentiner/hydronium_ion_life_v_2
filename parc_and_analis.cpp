...
int to_main_par_test(std::string file){
                    ...
                    for (int i = 0; i < jump_H.size(); ++i) {
                        ...
                        for (int j = 0; j < o_arr.size(); ++j) {
                            ...
                            if (((dist_to_prev_belong_O - dist_to_pos_O) > water_lentg) and (belong_O.get_name() != O->get_name())){
                                ...
                                glosar_O_of_jump_H[O->get_name()] = change_index;
...
int to_main_par_gz(std::string file){
    std::vector<int> life_ar;
    std::vector<int> frame_time_ar;
    float water_lentg = 0.96;
    int frame_time = 0;
    ...
    gzFile infile = gzopen(file.c_str(), "rb");
    if (!infile ) {
        std::cout << "Unccorect file input" << std::endl;
        return -1;
    }

    std::vector<char> char_line = readline(infile);
    char_line = readline(infile);
    while(!char_line.empty()){
        std::string tp(char_line.begin(), char_line.end());
